...
public class GEPanel extends JPanel implements Printable {
	...
	private boolean fillColorstate = false; // 색깔 변경시 그림 체크하고 바꾼건지, 색 변경하고 그리는건지 체크
	private boolean lineColorstate = false;
	private boolean cutState = false;

	public boolean isFillColorstate() {return fillColorstate;}
	public void setFillColorstate(boolean fillColorstate) {this.fillColorstate = fillColorstate;}
	public boolean isLineColorstate() {return lineColorstate;}
	public void setLineColorstate(boolean lineColorstate) {this.lineColorstate = lineColorstate;}
	...
	public GEPanel() {
		...
		lineColor = GEConstant.COLOR_LINE_DEFAULT;
		fillColor = GEConstant.COLOR_FILL_DEFAULT;
			...
			try {
				if (eDrawingState == EDrawingState.idle) {
					...
					if (getSelectedShape() == null) {
						// 도구가 Polygon이 아니면
						setSelectedShape(currentTool.getClass().newInstance());
						getSelectedShape().setLineColor(lineColor);
						getSelectedShape().setFillColor(fillColor);
						currentTransformer = new GEDrawer(getSelectedShape());
