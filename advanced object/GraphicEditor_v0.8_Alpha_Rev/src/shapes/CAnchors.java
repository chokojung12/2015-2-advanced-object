...
public class CAnchors implements Serializable {
	...
	public CAnchors(Rectangle bounds) {
		...
		for (EAnchorType eanchorType: EAnchorType.values()) {
			...
			shape.setFrame(getLocation(bounds, eanchorType), new Dimension(CConstans.ANCHOR_W, CConstans.ANCHOR_H));
			anchors.add(shape);
		}
	}
	...
	private Point getLocation(Rectangle bounds, EAnchorType eanchorType) {
		...
		int w = bounds.width;
		int h = bounds.height;
		int dw = CConstans.ANCHOR_W/2;
		int dh = CConstans.ANCHOR_H/2;
		...
		case RR: return new Point(x+w/2-dw, y-CConstans.ANCHOR_H*5-dh);
		default: return null;
		}
	}
	...
	public void setTransformedShape(AffineTransform affineTrnasform) {
		for (int i=0; i<anchors.size(); i++) {
			...
			anchor.setFrameFromCenter(x, y, x+CConstans.ANCHOR_W/2, y+CConstans.ANCHOR_H/2);
